...
// import { useNavigate, useLocation } from "react-router-dom"; 
// import { motion } from "framer-motion";
// import { getCurrentUser } from "../Services/userServices"; // Import getCurrentUser
...
//   // Fetch current user data and set premium status
//   useEffect(() => {
...
//                   isPremium 
//                     ? "bg-gradient-to-r from-yellow-500 to-orange-500 hover:bg-yellow-900 text-black cursor-default" 
...
const Header: React.FC = () => {
  ...
  // Fetch current user data and set premium status and profile picture
  useEffect(() => {
    const fetchCurrentUserData = async () => {
      const token = localStorage.getItem("token");
      if (token) {
