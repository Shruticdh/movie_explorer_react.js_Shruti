...
const AllMovies: React.FC = () => {
  ...
  // Enhanced fetch suggestions with better fuzzy matching
  const fetchSuggestions = async (query: string) => {
    ...
    try {
      ...
      const fuzzyResults = movieTitles
        ...
        .filter((result) => result.score > 25) // Lower threshold for more forgiving matching
        .sort((a, b) => b.score - a.score)
        .map((result) => result.title)
        .slice(0, 8);
  ...
  // Enhanced movie loading with better fuzzy search
  const loadMovies = async (query: string, genre: string, page: number) => {
    if (isSearching) return;
    ...
    try {
      ...
      if (query && query.length >= 2) {
        // Get both targeted search results and broader dataset for fuzzy matching
